'use client'

import { cn } from '@/lib/utils'
import * as React from 'react'
import { HiX } from 'react-icons/hi'
import { AnimatePresence, motion } from 'framer-motion'
import Script from 'next/script'
import { Button } from '@/components/ui/button'

export default function Consent() {
  const [consent, setConsent] = React.useState<boolean>(false)
  const [hideBanner, setHideBanner] = React.useState<boolean>(true)

  type Preferences = {
    ad_storage: 'granted' | 'denied'
    analytics_storage: 'granted' | 'denied'
    date_denied?: string
  }
  React.useEffect(() => {
    setTimeout(() => {
      if (localStorage.getItem('consentMode') === null) {
        setHideBanner(false)
        setConsent(false)
      } else {
        let consentMode: Preferences = JSON.parse(
          `${localStorage.getItem('consentMode')}`
        )
        setConsent(consentMode.analytics_storage === 'granted')
        if (consentMode.date_denied) {
          const currentDate = new Date()
          const timeDifference =
            currentDate.getTime() - new Date(consentMode.date_denied).getTime()
          const daysSinceDenied = Math.floor(
            timeDifference / (1000 * 60 * 60 * 24)
          )
          daysSinceDenied > 14 && localStorage.removeItem('consentMode')
        }
      }
    }, 6000)
  }, [])
  return (
    <>
      {consent === true && (
        <Script
          id="consupd"
          strategy="afterInteractive"
          dangerouslySetInnerHTML={{
            __html: `
              gtag('consent', 'update', {
                'ad_storage': 'granted',
                'analytics_storage': 'granted'
              });
              localStorage.setItem('consentMode', JSON.stringify({ad_storage: 'granted', analytics_storage: 'granted'}));
            `,
          }}
        />
      )}
      <AnimatePresence>
        {!hideBanner && (
          <motion.div
            initial={{
              opacity: 0,
              y: '100%',
            }}
            animate={{
              opacity: 1,
              y: 0,
            }}
            exit={{
              y: '100%',
            }}
            id="consent-banner"
            className={cn(
              'fixed bottom-0 z-10 grid w-full bg-slate-900 p-3 md:grid-cols-5'
            )}
          >
            <p className="prose prose-sm mx-auto my-4 px-6 text-left md:col-span-3 dark:prose-invert">
              At our company, we respect your privacy, plain and simple. If
              you&apos;re okay with it, we&apos;ll keep track of some stuff. We
              only gather this info to make your visit better. We only collect
              data if you say it&apos;s ok (because that&apos;s the right thing
              to do).
            </p>
            <div className="my-4 flex items-center justify-evenly md:col-span-2">
              <button
                className="absolute right-2 top-2"
                onClick={(e) => {
                  setHideBanner(true)
                }}
              >
                <HiX className="text-skin-neutral h-6 w-6" />
                <span className="sr-only">Close</span>
              </button>
              <Button
                variant={'link'}
                onClick={(e) => {
                  setHideBanner(true)
                  localStorage.setItem(
                    'consentMode',
                    JSON.stringify({
                      ad_storage: 'denied',
                      analytics_storage: 'denied',
                      date_denied: new Date(),
                    })
                  )
                }}
              >
                Deny All
              </Button>
              <Button
                onClick={() => {
                  setConsent(true)
                  setHideBanner(true)
                }}
                variant={'default'}
              >
                Accept All
              </Button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
    </>
  )
}
